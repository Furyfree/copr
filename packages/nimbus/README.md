...
[`nimbus.spec`](nimbus.spec) expects `nimbus-0.3.0-vendor.tar.gz`, containing the
full reviewed Nimbus source rooted at `nimbus-0.3.0/`, with modules generated by
...
before dispatching this repository's `build` operation with `package=nimbus`. Preserve LICENSE,
