...
The recipe selects [v0.5.3](https://github.com/Furyfree/nimbus/releases/tag/v0.5.3)
from commit `4635c6d22e940e6264f084fde03a930d782c3555`. It enforces declared
RPM sources, integrates UWSM launch/service handling, applies explicit 1Password
integration choices, and configures constrained greeter appearance authorization
through normal approved init/sync. Definitions require engine 0.5.3.

The verified archive SHA-256 is
`4885f72e5bdc6e8f31514d92be9bdc27277b15e6f5998fcbbc42675539163537`.
All 348 tagged files and executable bits match the release archive. Its 754
vendored files, including dependency license notices, are unchanged from 0.5.2.
Nimbus CI, definition validation and offline vendored release tests passed.
The complete offline packaging gate and an unprivileged, network-disabled
SRPM rebuild pass. The SRPM contains the exact published archive and reviewed
spec. Local native tests verify greeter-rule lifecycle and service timeout
...
The recipe selects [v0.5.4](https://github.com/Furyfree/nimbus/releases/tag/v0.5.4)
from Nimbus commit `b9df8a081d0f60d6dadfa51fb1f94076c6e21e64`. It simplifies
maintenance previews and approvals, scopes setup inspection, records private
version-attributed diagnostics, and corrects package/status counts.
...
The recipe selects [v0.5.5](https://github.com/Furyfree/nimbus/releases/tag/v0.5.5)
from commit `d0687535653e0045f818f62d2609cd4140e54d16`. It fixes targeted
1Password configuration on fresh installs by including managed parent directories
in Chezmoi apply. Guided setup shows a concise change summary; explicit `--diff`
prints the private diff without a pager. Conflict prompts and verification remain.

Archive SHA-256:
`de3b682f01b867a0c7ba2ff4fb9a0c4743db6bc6e9dd37e2cec58d93cc3a6a30`.
All 353 tagged files match the archive; vendored dependencies are unchanged from
0.5.4. Nimbus CI, vendored build/tests, local checks and focused race tests pass.
...
The recipe selects [v0.5.6](https://github.com/Furyfree/nimbus/releases/tag/v0.5.6)
from commit `91df8a9c272e0734f44d01e490f0234752bac117`. It fixes false
lockscreen repair failures when Noctalia saves a proportional screen adjustment.
Status recognizes an equivalent saved layout without restarting the shell.
Widget appearance, sizes and unrelated preferences remain verified.

Archive SHA-256:
`c67b8f4265755a376d37a566880389de785ab85dd2371f3d87858625d6aee286`.
All 353 tagged files match the source archive; vendored dependencies are unchanged
from 0.5.5. Local checks, definition validation, focused race tests and the
vendored release build/tests passed. Push CI encountered the previously recorded
...
[Release v0.5.7](https://github.com/Furyfree/nimbus/releases/tag/v0.5.7) selects
Nimbus commit `631f96a077dfd972e0c69ea5fbf6623986605734`. It fixes initial
Tailscale sign-in with local operator permission and adds the explicit DTU
eduroam certificate task for NetworkManager. No user credentials or global
trust anchors are installed by the task or RPM.

The reviewed source archive SHA-256 is
`a1729743a1d47776888895744d9c4cece9f74e99fa86dbfbb6982ba2ddb135e0`.
All 358 tagged files and executable bits match the archive. Module versions
are unchanged from 0.5.6. Nimbus local checks, definition validation, focused
race tests and the vendored release checks passed. Native browser sign-in,
enforcing SELinux and campus Wi-Fi remain user trials. COPR submission does
not establish build success or RPM availability.

## Version 0.5.9 release

[Release v0.5.9](https://github.com/Furyfree/nimbus/releases/tag/v0.5.9) selects
Nimbus commit `4be55e2b3825ebfd91dbb9095ba21bfb99a0e700`. It fixes two test
synchronization races without changing installer or terminal runtime behavior.
It includes the 0.5.7 Tailscale and DTU setup changes.

COPR build 10985459 failed the 0.5.7 foreground-restoration test after its
success marker. A separate interrupt-readiness race blocked the 0.5.8 source
workflow, which published no source release or COPR build. The new regression
checks retain their deadlines and verify both completion and interruption.

Archive SHA-256:
`4da5ba421be8bc5ea8c9212dbbdad7935c26c142a8cab0510c2f9ee2b6414619`.
All 358 tagged files and executable bits match the archive. Vendored source
and license notices are unchanged from 0.5.7. Local checks, repeated offline
Fedora regressions, integration tests and the vendored release workflow pass.
COPR submission does not establish build success or RPM availability.

## Version 0.5.10 release

[Release v0.5.10](https://github.com/Furyfree/nimbus/releases/tag/v0.5.10) selects
Nimbus commit `a36618114b1c5287dfe626d9eec0c372f7c4d899`. It corrects DTU
certificate verification when restorecon preserves a valid SELinux user field.
Native label verification distinguishes mismatches from failed inspection;
diagnostics identify specific content, metadata and labeling failures.

Archive SHA-256:
`4f0959aef43e994fb63a59f12f87e6d2c2255785b6abeab60f6d3cd9257e29d3`.
All 358 tagged files and executable bits match the archive. Vendored source and
license notices are unchanged from 0.5.9. Full local checks, focused DTU race
tests and the vendored release workflow pass. The owner confirmed the local
command verifies the existing desktop certificate; campus Wi-Fi is not tested.
COPR submission does not establish build success or RPM availability.

## Version 0.6.1 release

[Release v0.6.1](https://github.com/Furyfree/nimbus/releases/tag/v0.6.1) selects
Nimbus commit `94821f2bbe429965eb3d6b4c576ebde2e2ef709d`. It is the first
stable engine with the Paper Dark GRUB and Plymouth themes, the grouped kernel
menu (`09_nimbus_previous_kernels`, `30_previous_kernels_nimbus`), the
`grub-config`, `plymouth-theme` and `initramfs-rebuild` triggers, engine
channels, passphrase unlock with greetd auto-login, and the desktop-only
`nvidia-boot-display` drop-in. The definitions on `main` require it through
`min_engine = "0.6.1"`; the 0.5.10 engine upgrades itself before it reads
definitions.

Archive SHA-256:
`9bda818a1d6420654bb5a3103e068ebbbb6a14d6a3148f4c8142546f9eef959b`.
All 407 tagged files and executable bits match the archive. Go modules changed
since 0.5.10 (`charmbracelet/x/ansi`, `colorprofile`, `x/sys`, `x/term` and
their indirect dependencies); the licence expression is unchanged, and
`licenses/GEANT-CAT.txt` now ships as described below. The upstream gate and
the vendored release workflow pass, and the same source built as
`0.6.1~dev.20260920193406` in the develop project was installed and booted on
the owner's desktop. The laptop has not booted this engine yet.
COPR submission does not establish build success or RPM availability.

## Version 0.6.2 release

[Release v0.6.2](https://github.com/Furyfree/nimbus/releases/tag/v0.6.2) selects
Nimbus commit `408c35923f6a7faec55384aaae3269cb0af34af0`. The first `nimbus
sync` of this engine records a selector from before engine channels as schema
2 on the stable channel, shown in the preview and the closing report; the
one-time channel notice is removed. The definitions add `seahorse` to
`hyprland-session`; `min_engine` stays at 0.6.1.

Archive SHA-256:
`aabb46141732e8f311e939d31bf6fe00797100837c060c783b307640cc236f39`.
All 407 tagged files and executable bits match the archive. Vendored source and
license notices are unchanged from 0.6.1. The upstream gate and the vendored
release workflow pass. The migration has not yet run on a real schema 1
machine.
COPR submission does not establish build success or RPM availability.
...
[`nimbus.spec`](nimbus.spec) expects `nimbus-0.6.2-vendor.tar.gz`, containing the
full reviewed Nimbus source rooted at `nimbus-0.6.2/`, with modules generated by
