...
func ParseRelease(data []byte) (Artifact, error) {
	...
	if a.SchemaVersion != 1 || !stableVersion.MatchString(a.Version) || a.Name != "wowup-cf" || a.Arch != "x86_64" || a.Path != "" || !digestPattern.MatchString(a.SHA256) || a.Source != sourceURL(a.Version) {
		return a, errors.New("invalid packaged WoWUp release")
	}
	return a, nil
}
...
func (e *Installer) Install(ctx context.Context) (Status, error) {
	...
	if current.Installed {
		...
		if a.Version == current.Version {
			if a.SHA256 != current.SHA256 {
				return result, errors.New("installed WoWUp digest differs from the release selected by this package")
			}
	...
	if resolved != a {
		return result, errors.New("upstream metadata differs from the WoWUp release selected by this package")
	}
	stage, err := os.MkdirTemp(filepath.Dir(e.app), ".wowup-cf-download-")
	if err != nil {
		return result, err
	}
