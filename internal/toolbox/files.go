...
func (e *Installer) readReceipt(path string) (*Receipt, error) {
	...
	if !info.Mode().IsRegular() || info.Sys().(*syscall.Stat_t).Nlink != 1 || info.Mode() != 0o644 || info.Size() > 16*1024*1024 {
		return nil, errors.New("JetBrains Toolbox receipt is not a regular bounded 0644 file")
	}
	...
	if r.SchemaVersion != 1 || r.Owner != owner || !buildPattern.MatchString(r.Version) || !digestPattern.MatchString(r.SHA256) || r.Source != sourceURL(r.Version) || r.Entries == nil {
		return nil, errors.New("invalid JetBrains Toolbox receipt identity")
	}
	...
	if r.Entries[archiveName].Type != "file" || r.Entries[archiveName].SHA256 != r.SHA256 || r.Entries["app"].Type != "directory" || r.Entries["app/bin/jetbrains-toolbox"].Type != "file" || r.Entries["launcher"].Type != "file" || r.Entries["jetbrains-toolbox.desktop"].Type != "file" {
